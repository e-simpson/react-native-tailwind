...
function isColorClass(className) {
  return className.startsWith("text-") || className.startsWith("bg-") || className.startsWith("border-") || className.startsWith("outline-");
}
function expandSchemeModifier(schemeModifier, customColors, darkSuffix = "-dark", lightSuffix = "-light") {
  ...
  if (!isColorClass(baseClass)) {
    if (process.env.NODE_ENV !== "production") {
      console.warn(
        `[react-native-tailwind] scheme: modifier only supports color classes (text-*, bg-*, border-*, outline-*). Found: "${baseClass}". This modifier will be ignored.`
      );
    }
    return [];
  }
