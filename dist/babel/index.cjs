...
// src/babel/apply-loader.ts
var applyFileCache = /* @__PURE__ */ new Map();
...
function resolveBaseDir(filename) {
  const startDir = filename ? path2.dirname(filename) : process.cwd();
  const configPath = findTailwindConfig(startDir);
  return configPath ? path2.dirname(configPath) : process.cwd();
}
function resolveApplyFilePath(filePath, filename) {
  if (path2.isAbsolute(filePath)) {
    return path2.normalize(filePath);
  }
  const baseDir = resolveBaseDir(filename);
  return path2.resolve(baseDir, filePath);
}
function parseApplyTokens(body) {
  const applyTokens = [];
  let hasApply = false;
...
function mergeRegistryEntry(registry, className, tokens) {
  const existing = registry.get(className) ?? [];
  existing.push(...tokens);
  registry.set(className, existing);
}
function parseApplyCss(content, filePath) {
  ...
  for (const match of css.matchAll(blockRegex)) {
    ...
    for (const selector of selectors.split(",").map((part) => part.trim()).filter(Boolean)) {
      ...
      if (!className) {
        warn(
          `[react-native-tailwind] Unsupported selector "${selector}" in ${filePath}. Only simple class selectors are supported (e.g. .button).`
        );
        continue;
      }
...
function loadApplyFile(filePath) {
...
function loadApplyClassRegistry(options, filename) {
...
function resolveAlias(alias, registry, resolvedAliasCache, stack) {
...
function expandApplyClasses(className, registry, classNameCache, resolvedAliasCache) {
...
function createInitialState(options, filename, colorSchemeImportSource, colorSchemeHookName, schemeModifierConfig) {
  ...
  return {
    ...
    hasTwImport: false,
    applyClassRegistry,
    applyClassNameCache: /* @__PURE__ */ new Map(),
    applyAliasCache: /* @__PURE__ */ new Map(),
...
function findStyleAttribute(path3, targetStyleProp, t) {
...
function findComponentScope(path3, t) {
  let current = path3.getFunctionParent();
  while (current) {
...
function jsxAttributeVisitor(path3, state, t) {
  ...
  const splitModifierClassesWithApply = (className) => splitModifierClasses(
    expandApplyClasses(
      className,
  ...
  const processStaticClassName = (className) => {
    ...
    try {
      ...
      throw path3.buildCodeFrameError(error instanceof Error ? error.message : String(error));
    }
    const modifierClasses = [];
    for (const modifier of rawModifierClasses) {
    ...
    if (placeholderModifiers.length > 0) {
      const jsxOpeningElement = path3.parent;
    ...
    if (hasColorSchemeModifiers) {
      componentScope = findComponentScope(path3, t);
      if (componentScope) {
        state.functionComponentsNeedingColorScheme.add(componentScope);
      } else {
        if (process.env.NODE_ENV !== "production") {
    ...
    if (hasStateModifiers && (hasPlatformModifiers || hasColorSchemeModifiers || hasDirectionalModifiers)) {
      ...
      if (componentSupport) {
        ...
        const styleAttribute2 = findStyleAttribute(path3, targetStyleProp, t);
        if (styleAttribute2) {
          mergeStyleFunctionAttribute(path3, styleAttribute2, styleFunctionExpression, t);
        } else {
          replaceWithStyleFunctionAttribute(path3, styleFunctionExpression, targetStyleProp, t);
        }
        return true;
      } else {
      }
    ...
    if ((hasPlatformModifiers || hasColorSchemeModifiers || hasDirectionalModifiers) && !hasStateModifiers) {
      ...
      if (styleAttribute2) {
        ...
        path3.node.name = t.jsxIdentifier(targetStyleProp);
        path3.node.value = t.jsxExpressionContainer(styleExpression);
      }
      return true;
    }
    ...
    if (hasRuntimeDimensions(styleObject)) {
      ...
      const componentScope2 = findComponentScope(path3, t);
      if (componentScope2) {
        state.hasClassNames = true;
    ...
    state.styleRegistry.set(styleKey, styleObject);
    const styleAttribute = findStyleAttribute(path3, targetStyleProp, t);
    if (styleAttribute) {
      mergeStyleAttribute(path3, styleAttribute, styleKey, state.stylesIdentifier, t);
    } else {
      replaceWithStyleAttribute(path3, styleKey, targetStyleProp, state.stylesIdentifier, t);
    }
    return true;
  };
  ...
  if (t.isJSXExpressionContainer(value)) {
    ...
    try {
      const componentScope = findComponentScope(path3, t);
      const result = processDynamicExpression(
        expression,
        state,
        parseClassName,
      ...
      if (result) {
        ...
        const styleAttribute = findStyleAttribute(path3, targetStyleProp, t);
        if (styleAttribute) {
          mergeDynamicStyleAttribute(path3, styleAttribute, result, t);
        } else {
          replaceDynamicWithStyleAttribute(path3, result, targetStyleProp, t);
        }
        return;
      }
    } catch (error) {
      if (error instanceof Error && error.message.includes("Circular @apply class reference")) {
        throw path3.buildCodeFrameError(error.message);
      }
      if (process.env.NODE_ENV !== "production") {
        console.warn(
...
function importDeclarationVisitor(path3, state, t) {
  const node = path3.node;
  if (node.source.value === "react-native") {
    ...
    state.reactNativeImportPath = path3;
  }
...
function addStyleSheetImport(path3, t) {
  const body = path3.node.body;
  ...
  if (existingValueImport) {
    ...
    path3.unshiftContainer("body", importDeclaration);
  }
}
function addPlatformImport(path3, t) {
  const body = path3.node.body;
  let existingValueImport = null;
  for (const statement of body) {
    if (t.isImportDeclaration(statement) && statement.source.value === "react-native") {
...
function addColorSchemeImport(path3, importSource, hookName, t) {
  const body = path3.node.body;
  let existingValueImport = null;
  for (const statement of body) {
...
function addI18nManagerImport(path3, t) {
  const body = path3.node.body;
  let existingValueImport = null;
  for (const statement of body) {
    if (t.isImportDeclaration(statement) && statement.source.value === "react-native") {
...
function injectI18nManagerVariable(path3, variableName, localIdentifier, t) {
  const body = path3.node.body;
  for (const statement of body) {
    if (t.isVariableDeclaration(statement) && statement.declarations.length > 0 && t.isVariableDeclarator(statement.declarations[0])) {
      const declarator = statement.declarations[0];
...
function addWindowDimensionsImport(path3, t) {
  const body = path3.node.body;
  let existingValueImport = null;
  for (const statement of body) {
    if (t.isImportDeclaration(statement) && statement.source.value === "react-native") {
...
function injectStylesAtTop(path3, styleRegistry, stylesIdentifier, t) {
...
function processTwCall(className, path3, state, parseClassName2, generateStyleKey2, splitModifierClasses2, findComponentScope2, t) {
  const { baseClasses, modifierClasses: rawModifierClasses } = splitModifierClasses2(className);
  const modifierClasses = [];
  for (const modifier of rawModifierClasses) {
    if (isSchemeModifier(modifier.modifier)) {
  ...
  let componentScope = null;
  if (hasColorSchemeModifiers) {
    componentScope = findComponentScope2(path3, t);
    if (!componentScope) {
      if (process.env.NODE_ENV !== "production") {
        console.warn(
  ...
  path3.replaceWith(twStyleObject);
}
function removeTwImports(path3, t) {
  path3.traverse({
...
function programExit(path3, state, t) {
  if (state.hasTwImport) {
    removeTwImports(path3, t);
  }
  ...
  if (!state.hasStyleSheetImport && state.styleRegistry.size > 0) {
    addStyleSheetImport(path3, t);
  }
  if (state.needsPlatformImport && !state.hasPlatformImport) {
    addPlatformImport(path3, t);
  }
  if (state.needsI18nManagerImport && !state.hasI18nManagerImport) {
    addI18nManagerImport(path3, t);
  }
  if (state.needsI18nManagerImport) {
    injectI18nManagerVariable(path3, state.i18nManagerVariableName, state.i18nManagerLocalIdentifier, t);
  }
  if (state.needsColorSchemeImport && !state.hasColorSchemeImport) {
    addColorSchemeImport(path3, state.colorSchemeImportSource, state.colorSchemeHookName, t);
  }
  ...
  if (state.needsWindowDimensionsImport && !state.hasWindowDimensionsImport) {
    addWindowDimensionsImport(path3, t);
  }
  ...
  if (state.styleRegistry.size > 0) {
    injectStylesAtTop(path3, state.styleRegistry, state.stylesIdentifier, t);
  }
}

// src/babel/plugin/visitors/tw.ts
function taggedTemplateVisitor(path3, state, t) {
  const node = path3.node;
  ...
  if (!className) {
    path3.replaceWith(t.objectExpression([t.objectProperty(t.identifier("style"), t.objectExpression([]))]));
...
function callExpressionVisitor(path3, state, t) {
  const node = path3.node;
    ...
    visitor: {
      Program: {
        enter(path3, state) {
          ...
          programEnter(path3, state);
        },
        exit(path3, state) {
          programExit(path3, state, t);
        }
      },
      ImportDeclaration(path3, state) {
        importDeclarationVisitor(path3, state, t);
      },
      TaggedTemplateExpression(path3, state) {
        taggedTemplateVisitor(path3, state, t);
      },
      CallExpression(path3, state) {
        callExpressionVisitor(path3, state, t);
      },
      JSXAttribute(path3, state) {
        jsxAttributeVisitor(path3, state, t);
      }
    }
  };
}
